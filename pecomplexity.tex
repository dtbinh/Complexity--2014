...
results from the dimension, nonlinearity, and non-stationarity of the
generating process, as well as from measurement issues like noise,
aggregation, and finite data length --- is both empirically
quantifiable and directly correlated with predictability.  In
particular, we argue that \emph{redundancy} is an effective way to
measure complexity and predictive structure in a time series and that
\emph{weighted permutation entropy} is an effective way to estimate
that complexity.  To validate these conjectures, we study 120
...
series.  We use the results to develop a heuristic that can help
practitioners recognize when a particular prediction method is not
well matched to the task at hand: that is, when the time series has
more predictive structure than that method can capture and exploit.
...
We would also like to acknowledge the Santa Fe Institute whose interdisciplinary halls created a place for this work to be born.
