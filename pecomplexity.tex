...
conjecture that the inherent complexity of a time series—which re-
sults from the inherent dimension, nonlinearity, and non-stationarity
of the generating process, as well as from measurement issues like
noise, aggregation, and finite data length—is both empirically
quantifiable and directly correlated with predictability. To validate
this conjecture, we study 120 different time-series data sets and a
wide variety of forecast models. We estimate the complexity of each
data set using a variant of permutation entropy, then compare that
complexity with the accu- racy of each forecast method. We use the
results to develop a heuristic that can help practitioners recognize
when a particular prediction method is not well matched to the task at
hand: that is, when the time series has more predictive structure than
that method can capture and exploit.
