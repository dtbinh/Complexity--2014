...
method.  The position of the LMA cluster, for instance---well to the
...
time series indicates that it is nonstationary, an integration term is
added to the model.  According to this test, the \col time series is
right on the edge of nonstationarity, so the ARIMA procedure sometimes
omits the integration term, which increases the error, thereby
spreading out the points.  

The WPE of \svdfive ($0.677 \pm 0.006$) is higher than that of {\tt
  col\_major}.  This indicates that the rate{\color{red} is this
  really a rate} of forward information transfer of the underlying
process is lower, but that time-series data from this system still
contain a significant amount of structure that can, in theory, be
leveraged to predict the future course of the time series.  As
mentioned above, though, the effectiveness of any prediction strategy
depends on how well it leverages the available information: methods
...
\pm 0.016$), two of the three prediction methods do quite well,
yielding $MASE$ scores of $0.827 \pm 0.076$ (LMA) and $0.714 \pm
...
the trend followed by the other seven clusters.  The $MASE$ scores of
the predictions of this system produced by the \naive ~method,
however, are highly inconsistent: $2.676 \pm 4.328$.  The majority of
the blue diamond-shaped points on the top plot in the figure are
clustered near a $MASE$ score of 0.6, but three of the 15 \svdone
trials posed challenges to this prediction method, resulting in $MASE$
scores $\approx 2.5$. {\color{red} this is becasue of the weird split it does some of the time. Should this be said?}

These effects, we believe, are artifacts of the way $MASE$ is
calculated, as well as the nonstationarity properties of the \svdone
signal.  Recall that $MASE$ scores are scaled \emph{relative to a
  random-walk forecast}, which uses the current value as the
prediction.  This strategy works very badly on signals with frequent,
large, rapid transitions.  Consider a signal that oscillates from one
end of its range to the other at every step.  A signal like this will
have a low WPE, much like {\tt col\_major}.  However, a random-walk
forecast of this signal will be 180 degrees out of phase with the true
...
vs. $MASE$ plot, and that is exactly why the \svdone clusters for
ARIMA and LMA are above the dashed line in
...
is dominated by a white-noise{\color{red} can we actually say white-noise here} regime (between $\approx$1.86 and
...
the \svdone $MASE$ values.  {\color{red} We need to think about the
  three outliers and put in a sentence here about them., these are the dip ones}  For all of
these reasons, we view \svdone as an outlier and exclude it from the
fit calculation of the dashed line in
