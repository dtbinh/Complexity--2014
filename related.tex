...
not to add a new weapon to this arsenal, nor to assess or compare the
effectiveness of existing methods.  Our goals are more general:
\emph{(i)} to empirically quantify the predictive structure that is
present in a real-valued scalar time series and \emph{(ii)} to explore
...
beginning with a few points and working up to a global linear fit
...
and stochasticity present in a time series.\alert{Liz Should we say what this has to do with predictability, realized this paragraph may seem unrelated to a casual reader}
...
We sidestep these issues by using a variant of the \emph{permutation
  entropy} of Bandt and Pompe~\cite{bandt2002per} to estimate the value
of the Kolmogorov-Sinai entropy of a real-valued time series---and
thus the redundancy in that data, which we show to be an effective proxy for
