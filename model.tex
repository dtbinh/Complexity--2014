...
\alert{we had 360 there.  seems like it should be 8 times 15 which is
  120.}  traces described in Section~\ref{sec:methods}, then calculate
the error of the predictions with respect to the true continuations.
...
fact that the values in the second-from-\alert{left} column are not
...
signal---was easier, on the average, for RW to predict than the first
90\%. \alert{[[Liz to do: add pointer to Josh's paragraph for deeper
      analysis]]}.
