...
\subsection{Seasonal Autoregressive-integrated-moving average(ARIMA)}\label{sec:arima}
\begin{enumerate}
\item introduce method abstractly and practically, common method basically fitting a hyperplane to data removing seasonality and filtering for noise
\item define rigorously, all the backshift operator garbage

$$\Phi(B^m)\phi(B)(1-B^m)^D(1-B)^dX_i = c + \Theta(B^m)\theta(B)\epsilon_i$$

$$B^mX_i  = X_{i-m}$$
\item discuss that this method needs linear structure to work correctly
\item maybe talk about 
\end{enumerate}
