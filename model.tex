...
structure, but those variations are not---on the average---very large, so
...
LMA method, which can generate arbitrary-length predictions (although these predictions will degrade over time in the presence of positive Lyapunov exponents), but the
performance of the other three methods used here will degrade severely if the
