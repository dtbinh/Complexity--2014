...
\item The \emph{ARIMA} (auto-regressive integrated moving average)
  ...
  \emph{AutoARIMA} procedure \cite{arima}, and
...
situations described in the previous paragraph.  The AutoARIMA and LMA
...
accomplish this, producing what we call an \emph{AutoARIMA model}.
...
free parameters of the AutoARIMA model.
...
build forecasts in a stepwise fashion: i.e., fit the AutoARIMA model
to the existing data, use that model to perform a one-step prediction,
rebuild it using the latest observations, and iterate until the
desired prediction horizon is reached.  (For consistency, we take the
same approach with the other three models in this study as well, even
...
\begin{table*}
  ...
  \begin{tabular}{lccc}
    ...
    \begin{sideways}\large AutoARIMA\end{sideways} &
...
generates a very accurate prediction of the \col data, while AutoARIMA
...
two-dimensional embedding with $\tau=1$.  For AutoARIMA, the
...
effect is largely the same\footnote{The temporal \emph{ordering} of
  the points on the AutoARIMA $p_j$ vs. $c_j$ plot does not match that of
...
absolute scaled error (MASE) scores and weighted permutation entropies for all eight
  ...
  \hline\hline Signal & LMA MASE  & AutoARIMA MASE & na\"{i}ve MASE  & Weighted Permutation Entropy \\
  \hline
...
$0.050$, for instance, while AutoARIMA scored much worse ($0.599$).
That is, LMA performed roughly 20 times better on \col signals than a
random-walk predictor, while AutoARIMA only outperformed random walk
