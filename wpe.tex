...
the fact that the signal is actually quite structed. The weighted 
...
\begin{align*}
  w(x_{t+1}^\ell) = \frac{1}{\ell} \sum_{i = t}^{t+\ell}
                      \left( x_i - \bar{x}_{t+1}^\ell \right)^2
\end{align*}
where $x_{t+1}^\ell$ is a sequence of values $x_{t+1}, \ldots,
x_{t+\ell}$, and $\bar{x}_{t+1}^\ell$ is the arithmetic mean of
those values.

The weighted probability of a permutation is defined as:
\begin{align*}
  P_w(\pi) = \frac{\displaystyle \sum_{t \le T - \ell} w(x_{t+1}^\ell) \cdot \delta(\phi(x_{t+1}^\ell), \pi) }{\displaystyle \sum_{t \le T - \ell} w(x_{t+1}^\ell)}
\end{align*}
...
system states evolved from small ensembles predicts the future of a 
