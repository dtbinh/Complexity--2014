...
is the aforementioned \emph{Shannon entropy rate}.  It seems obvious
that the more redundancy in a signal, the more predictable it is.
...
instance.  This issue---which has been explored extensively using
clean data and/or systems where the generating process is known, but
not so much with empirical data from unknown systems---is central to
the claims in this paper and the discussion in the following section.
...
two distributions: one evolved from a small ensemble of past
states using the known dynamical system, and the other the
observed distribution. Our work quantifies the predictability of a
